...
export function SiteNavigation({ active }: { active: ActivePage }) {
  ...
  return <>
    <aside className={`site-sidebar ${open ? 'open' : ''}`} aria-label="روابط رئيسية">
      <div className="site-sidebar-header"><button className="site-close" onClick={() => setOpen(false)} aria-label="إغلاق القائمة">×</button><a href="/" className="site-brand"><span><img src="icons/favicon-white.svg" alt="" /></span><strong>على رفوف المساجد<small>مكتبات المساجد في مكان واحد</small></strong></a></div>
    ...
    <header className="site-topbar"><div className="site-topbar-inner"><button className="site-menu-button" onClick={() => setOpen(true)} aria-label="فتح القائمة">☰</button><a href="/" className="site-brand"><span><img src="icons/favicon-white.svg" alt="" /></span><strong>على رفوف المساجد<small>مكتبات المساجد في مكان واحد</small></strong></a>{user ? <a className="site-user-link" href="/profile"><span className="site-user-avatar">{avatar ? <img src={avatar} alt="" /> : name[0]}</span><span>{name}</span></a> : <a className="site-login-link" href={`/login?redirect=${active === 'profile' ? '/profile' : active === 'browse' ? '/' : `/${active}`}`}>تسجيل الدخول</a>}</div></header>
  </>
}