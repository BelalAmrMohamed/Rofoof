...
export function SiteNavigation({ active }: { active: ActivePage }) {
  ...
  return <>
    <aside className={`site-sidebar ${open ? 'open' : ''}`} aria-label="روابط رئيسية">
      <div className="site-sidebar-header"><button className="site-close" onClick={() => setOpen(false)} aria-label="إغلاق القائمة">×</button><a href="/" className="site-brand"><span><img src="/book.svg" alt="" /></span><strong>على رفوف المساجد<small>مكتبات المساجد في مكان واحد</small></strong></a></div>
