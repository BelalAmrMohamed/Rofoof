...
const items: { key: ActivePage; label: string; icon: 'book' | 'plus' | 'requests' | 'user' | 'info'; href: string; mobileHide?: boolean }[] = [
  { key: 'browse',   label: 'تصفح',        icon: 'book',     href: '/' },
  ...
  // Profile is accessible from the header avatar — hidden from the mobile bottom tab bar
  { key: 'profile',  label: 'حسابي',       icon: 'user',     href: '/profile', mobileHide: true },
]
    ...
    <>
      <aside className={`site-sidebar ${open ? 'open' : ''}`} aria-label="روابط رئيسية">
        <div className="site-sidebar-header">
          <button className="site-close" onClick={() => setOpen(false)} aria-label="إغلاق القائمة">×</button>
        ...
        <nav className="site-nav-list">
          {items.map((item) => (
            <button
              key={item.key}
              data-key={item.key}
              className={`site-nav-item ${active === item.key ? 'active' : ''} ${item.mobileHide ? 'mobile-hide' : ''}`}
      ...
      <header className="site-topbar">
        <div className="site-topbar-inner">
          <button className="site-menu-button" onClick={() => setOpen(true)} aria-label="فتح القائمة">☰</button>
          <a href="/" className="site-brand">
            <span><img src="/favicon.svg" alt="" /></span>
            <strong>على رفوف المساجد<small>مكتبات المساجد في مكان واحد</small></strong>
          </a>
          {user
            ...
            : <a className="site-login-link" href={`/login?redirect=${active === 'profile' ? '/profile' : active === 'browse' ? '/' : `/${active}`}`}>تسجيل الدخول</a>
          }
        </div>
      </header>
    </>
