...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (result.error) setNotice({ type: 'error', text: result.error.message })
        ...
        setDraftGov(result.data.governorate ?? '')
        setDraftCity(result.data.city ?? '')
        setDraftPoint(result.data.lat && result.data.lng ? { lat: result.data.lat, lng: result.data.lng } : null)
  ...
  const saveLocation = async () => {
    ...
    const result = await supabase.from('users').update({ governorate: draftGov, city: draftCity, lat: draftPoint?.lat ?? null, lng: draftPoint?.lng ?? null, location_source: draftPoint ? 'manual_pin' : 'skipped' }).eq('user_id', (await supabase.auth.getUser()).data.user?.id ?? '')
    if (result.error) setNotice({ type: 'error', text: result.error.message })
  ...
  if (loading) return <div className="site-layout" dir="rtl"><SiteNavigation active="profile" /><div className="site-content"><main className="profile-page"><p className="profile-message">جارٍ تحميل الملف الشخصي...</p></main></div></div>
